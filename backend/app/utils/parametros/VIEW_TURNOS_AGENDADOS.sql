...
SELECT
	CI.ID_PACIENTE,
	CI.ID_CITA,
	REPLACE(INITCAP(PE.PRIMER_NOMBRE || ' ' || COALESCE(PE.SEGUNDO_NOMBRE, '') || ' ' || COALESCE(PE.TERCER_NOMBRE, '')  || ' ' || PE.PRIMER_APELLIDO || ' ' || COALESCE(PE.SEGUNDO_APELLIDO, '')), '  ', '') PACIENTE,
	...
	REPLACE(INITCAP(PE_PRO.PRIMER_NOMBRE || ' ' || COALESCE(PE_PRO.SEGUNDO_NOMBRE, '') || ' ' || COALESCE(PE_PRO.TERCER_NOMBRE, '')  || ' ' || PE_PRO.PRIMER_APELLIDO || ' ' || COALESCE(PE_PRO.SEGUNDO_APELLIDO, '')), '  ', '') PROFESIONAL
FROM CITAS CI
...
WHERE TO_TIMESTAMP(TO_CHAR(CI.FECHA, 'dd/mm/yyyy') || ' ' || TO_CHAR(CI.HORA, 'hh24:mi'), 'dd/mm/yyyy hh24:mi') >= NOW() AND CI.ESTADO = 1
