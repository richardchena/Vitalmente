    ...
    <div>
        <nav class="navbar" style="background: #c6c6c6">
            <div style="margin-left: 15px">
                <strong>Modificar datos del timbrado</strong>
            </div>

            <div class="d-flex flex-row align-items-center justify-content-center">
                <div style="margin-right: 10px">
                    <button class="btn btn-info text-white" @click="validar">
                        MODIFICAR
                    </button>
                </div>
                <div style="margin-right: 10px">
...
export default {
    ...
    methods: {
        async datos_iniciales(){
            ...
            this.nro = data.nro_timbrado

            if(!this.nro){
                this.$router.push({path: '/'})
            }

            this.est = data.establecimineto
            this.exp = data.expedicion
            this.selectedRuc = data.contribuyente
            this.rango_inicio = data.nro_desde
        ...
        async modificar(obj){
            ...
            try {
                const {data} = await authApi.put(`/pagos/timbrado`, obj)

                if(data.id === 0) {
                    Swal.fire({
                    html: `<h4>Se ha modificado correctamente el timbrado #${obj.nro}</h4>`,
                    icon: 'success'})
                    .then(() => {
                        this.$router.push({ name: 'timbrados' })
                    })
